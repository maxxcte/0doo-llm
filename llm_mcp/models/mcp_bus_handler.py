...
class MCPBusHandler(models.AbstractModel):
    ...
    def handle_mcp_message(self, server_id, message_type, message_data):
        ...
        try:
            if message_type == 'response':
                ...
                _logger.warning(f"Received unknown message type from MCP server {server_id}: {message_type}")

        except Exception as e:
            _logger.exception(f"Error handling MCP message from server {server_id}: {e}")
    ...
    def _process_mcp_notification(self, server_id, notification):
        ...
        if not isinstance(notification, dict) or 'method' not in notification:
            _logger.warning(f"Received invalid notification format from server {server_id}")
            return
    ...
    def _handle_tool_state_changed(self, server_id, params):
        ...
        try:
            ...
            _logger.error(f"Error handling tool state change for server {server_id}: {e}")
    ...
    def _handle_server_state_changed(self, server_id, params):
        ...
        try:
            ...
            _logger.error(f"Error handling server state change for server {server_id}: {e}")
