...
class LLMDocumentEmbedder(models.Model):
    ...
    def action_reindex(self):
        ...
        chunks_with_embeddings = self.chunk_ids.filtered(
        ...
        try:
            # Post status message
            self._post_message("Starting re-indexing process...", "info")
            ...
            # Post error message
            self._post_message(f"Error during re-indexing: {str(e)}", "error")
            raise UserError(_("Re-indexing failed: %s") % str(e)) from e
    ...
    def action_mass_reindex(self):
        ...
        documents_with_chunks = self.filtered(
            lambda d: d.chunk_ids
            and d.chunk_ids.filtered(lambda c: c.embedding is not None)
        ...
        for model_id, docs in docs_by_model.items():
            try:
                ...
                _logger.error(
                    f"Error reindexing documents for model {model_id}: {str(e)}"
                )

        # Show a notification with the results
        message = (
            f"Reindexing complete: {success_count} successful, {error_count} failed."
        )
        return {
