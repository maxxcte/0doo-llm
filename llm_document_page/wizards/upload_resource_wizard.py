...
class UploadResourceWizardInherit(models.TransientModel):
    ...
    def _process_external_urls(self, collection, attachment_model_id, file_count):
        ...
        if not self.download_as_document_page:
            return super()._process_external_urls(collection, attachment_model_id, file_count)
        ...
        document_page_model_id_rec = IrModel.search([("model", "=", "document.page")], limit=1)
        if not document_page_model_id_rec:
            _logger.warning("document.page model not found. Falling back to ir.attachment creation.")
        ...
        for index, url in enumerate(urls):
            ...
            try:
                ...
                _logger.error(f"Failed to create document.page for URL {url}: {e}", exc_info=True)
            ...
            try:
                ...
                _logger.error(f"Failed to create llm.resource for URL {url} (doc_page: {doc_page.id}): {e}", exc_info=True)
