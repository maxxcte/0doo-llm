...
class ModelInspectorWizard(models.TransientModel):
    ...
    def action_inspect_model(self):
        ...
        try:
            ...
            raise UserError(
                _("Model '%s' not found in Odoo environment.") % model_technical_name
            )
        ...
        for name, member in reversed(members):
            ...
            details = self._extract_method_details(
                model_cls, member, name
            )  # Pass model_cls
            if details:
    ...
    def _extract_method_details(self, model_cls, method_obj, name):
        ...
        try:
            ...
            has_positional_only = any(
                p.kind == parameter_kinds.POSITIONAL_ONLY
                for p in sig.parameters.values()
            )
            has_keyword_only = any(
                p.kind == parameter_kinds.KEYWORD_ONLY for p in sig.parameters.values()
            )
            ...
            for i, param in enumerate(sig.parameters.values()):
                ...
                if param.kind == parameter_kinds.VAR_POSITIONAL:
                    ...
                    if not added_var_positional_marker:
                        is_first_kwonly = all(
                            p.kind != parameter_kinds.KEYWORD_ONLY
                            for p in list(sig.parameters.values())[:i]
                        )
                        if is_first_kwonly:
                            param_parts.append("*")
                ...
                if param.annotation is not inspect.Parameter.empty:
                    try:
                        ...
                        annot_repr = getattr(
                            param.annotation, "__name__", None
                        )  # For simple types/classes
                        if annot_repr is None:
                            annot_repr = getattr(
                                param.annotation, "_name", None
                            )  # For typing._GenericAlias like List, Union
                        if annot_repr is None:
                ...
                if param.kind == parameter_kinds.POSITIONAL_ONLY:
                    ...
                    is_last_posonly = all(
                        p.kind != parameter_kinds.POSITIONAL_ONLY
                        for p in list(sig.parameters.values())[i + 1 :]
                    )
                    if is_last_posonly:
                        param_parts.append("/")
            ...
            if sig.return_annotation is not inspect.Signature.empty:
                try:
                    ...
                    if ret_annot_repr is None:
                        ret_annot_repr = repr(sig.return_annotation).replace(
                            "typing.", ""
                        )
        ...
        if name == "serialize_model_data":
            _logger.info(
                f"DEBUG CHECK for '{name}': Type is {type(method_obj)}, Is staticmethod? {isinstance(method_obj, staticmethod)}"
            )
        ...
        if method_api == "model":
            ...
            decorators_list.append(
                "@api.model_create_multi or @api.model_create_single"
            )
        ...
        # Fallback to previous logic if not detected via dict (less likely needed now)
            ...
            _logger.warning(
                f"Detected staticmethod '{name}' via isinstance fallback, not getattr_static."
            )
            details["method_type"] = "static"
            decorators_list.append("@staticmethod")
        elif isinstance(method_obj, classmethod):
            _logger.warning(
                f"Detected classmethod '{name}' via isinstance fallback, not getattr_static."
            )
            details["method_type"] = "class"
            decorators_list.append("@classmethod")
        else:
            ...
            if hasattr(method_obj, "__get__") and not isinstance(
                method_obj, (staticmethod, classmethod)
            ):
...
class ModelInspectorWizardLine(models.TransientModel):
    ...
    wizard_id = fields.Many2one(
        "llm_tool.model.inspector.wizard",
        string="Wizard",
        required=True,
        ondelete="cascade",
    )
